...
def generate_complex_audio_single_period(sample_rate, waveforms):
    complex_audio = np.zeros(0)
    for waveform in waveforms:
        single_period_waveform = generate_waveform_single_period(sample_rate, **waveform)
        complex_audio = np.concatenate((complex_audio, single_period_waveform))
...
def main():
    ...
    for i in tqdm(range(num_samples)):
        ...
        complex_audio = generate_complex_audio_single_period(sample_rate, waveforms)
    ...
    with open('complex_audio_data_one_period.json', 'w') as f:
        json.dump(data, f)
